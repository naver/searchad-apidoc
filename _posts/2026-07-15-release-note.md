...
- 광고그룹 등록/수정 요청과 조회 응답에 **`aiAdsOptIn`**(ADVoost Max 설정) 필드가 추가됩니다.
- **파워링크 + 확장검색**을 사용하는 광고그룹만 설정/활성화할 수 있습니다.
- 값을 지정하지 않으면 **true**(사용)로 처리됩니다.

### 광고그룹 등록

* `POST /ncc/adgroups` 요청으로 `aiAdsOptIn` 값을 설정합니다.
...
* An **`aiAdsOptIn`** (ADVoost Max) field is added to the Ad Group create/update requests and the retrieval response.
...
* Set the `aiAdsOptIn` value with the `POST /ncc/adgroups` request.
