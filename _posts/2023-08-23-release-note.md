...
#### 2023년 8월 23일, 파워링크 캠페인 광고 그룹에 잠재고객타게팅(BETA) 기능이 추가됩니다.([공지 보기](https://saedu.naver.com/notice/view.naver?notiSeq=4151))


### 기존 광고 객체 변경
* Criterion 광고 객체의 유형에 `잠재 고객(Beta)` 이 추가 되었습니다.
* Criterion 광고 객체는 `지역`, `요일시간`, `성별`, `연령(대)`, `잠재 고객(Beta)` 타겟팅을 지원합니다.
* Criterion
  ```javascript
  {
...
* Dictionary Type
  ...
  enum DictionaryType {
    ...
    AUDIENCE = 'AD', // 잠재 고객(Beta)
  }
  ```

### 타겟팅 변경 사항
* 기존 `지역`, `요일시간`, `성별`, `연령(대)` 변경 API에 , `잠재 고객(Beta)`가 추가 제공됩니다.
* 광고그룹 유형별 지원되는 타겟팅과 입찰가 가중치 지원 여부는 아래와 같습니다.

  광고그룹 유형 | 요일/시간 | 지역 | 성별 | 연령대 | 잠재 고객(Beta)
  ----------|---------|-----|-----|------|-----
...
#### 잠재고객(Beta) 노출 설정
* 잠재고객(Beta)의 노출 설정을 변경할 수 있습니다.
* negative Parameter의 값이 false일 경우 해당 잠재 고객에 대해서는 광고 노출이 되지 않습니다.
* Request
  ```shell
